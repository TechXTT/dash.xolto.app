...
export default function AdminPage() {
  ...
  const aiUsers = useMemo(() => users.filter((entry) => entry.ai_call_count > 0).length, [users]);
  const topAIUser = useMemo(
    () => [...users].sort((a, b) => b.ai_tokens - a.ai_tokens)[0] ?? null,
    [users],
  );
  const failureRate =
    stats && stats.TotalCalls > 0 ? (stats.FailedCalls / stats.TotalCalls) * 100 : 0;
    ...
    return (
      <div className="page-stack">
        <section className="surface-panel empty-state">
          ...
          <p>
            Sign in with an email listed in <code>ADMIN_EMAILS</code> to open this workspace.
          </p>
        </section>
      </div>
    );
      ...
      <section className="hero-panel compact">
        <div>
          ...
          <p className="hero-copy">
            This view follows the same mission dashboard patterns, but pivots the data toward
            platform health: who is active, how much AI is being used, and whether failures or spend
            are trending up.
          </p>
          <p className="hero-copy">
            Primary admin operations now live in{' '}
            <a href="https://admin.xolto.app" target="_blank" rel="noreferrer">
              admin.xolto.app
            </a>
            .
          </p>
    ...
    <section className="surface-panel">
      <div className="section-heading">
        <div>
          ...
          <p className="section-support">
            Newest accounts first, with mission/search footprint and AI usage rolled into a single
            row.
          </p>
        </div>
        <div className="admin-note-meta">
    ...
    <section className="surface-panel">
      <div className="section-heading">
        ...
        <div className="admin-note-meta">
          ...
          <span className="subtle-pill">
            {entries.filter((entry) => !entry.Success).length} failures
          </span>
        </div>
      </div>
          ...
          <tbody>
            {entries.map((entry) => (
              <tr key={entry.ID} className={entry.Success ? '' : 'row-error'}>
                ...
                <td>
                  <code className="admin-inline-code">{entry.UserID || 'system'}</code>
                </td>
                <td>
                  <div className="admin-stack-cell">
                ...
                <td>
                  {entry.Success ? (
                    ...
                    <span className="warning-badge" title={entry.ErrorMsg || 'Unknown error'}>
                      Failed
                    </span>
                  )}
                </td>
